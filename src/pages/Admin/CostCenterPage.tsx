...
export const CostCenterPage = () => {
    const [centers, setCenters] = useState<CentroCusto[]>([]);
    const [newName, setNewName] = useState('');
    const [searchTerm, setSearchTerm] = useState('');

    const loadData = async () => {
        const res = await api.get<CentroCusto[]>('/centros-custo');
        setCenters(res.data);
    };
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        await api.post('/centros-custo', { nome: newName.toUpperCase(), ativo: true });
        setNewName('');
        loadData();
    };

    const handleToggleStatus = async (id: number) => {
        const msg = "Deseja alterar o status deste Centro de Custo?";
        if (window.confirm(msg)) {
            // Chamamos o endpoint de inativação que você configurou
            await api.patch(`/centros-custo/${id}/inativar`);
            loadData();
        }
    };
    ...
    return (
        <Box sx={{ maxWidth: 800, mx: 'auto' }}>
            <Typography variant="h4" sx={{ mb: 3 }}>Centros de Custo</Typography>
            ...
            <TableContainer component={Paper}>
                <Table>
                    <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                        <TableRow>
                            <TableCell>Nome do Centro de Custo</TableCell>
